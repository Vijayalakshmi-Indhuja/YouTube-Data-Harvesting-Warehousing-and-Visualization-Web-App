...
page_title=(':red[Youtube Data Harvesting and Warehousing | By Vijayalakshmi K]')
...
background-image: url("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTpFBjDFyg9GUwOfHLCjs2TjfyG_SCbyU_B0Q&usqp=CAU");
background-size: 170%;
...
with col1:
    ...
    st.write ('(Note:- By using **channel ID**s,this zone **collects data** and **stores it in the :green[MongoDB] database**.)')
    channel_id = st.text_input('**Enter 11 digit channel_id below**')
    ...
    if Get_data or st.session_state.Get_state:
        ...
        api_version = 'v3'
        api_key = 'AIzaSyB9AhfnsYQcpotOf0EEaa-u22RIGYKqvmg'
...
# create a client instance of MongoDB
        client = pymongo.MongoClient('mongodb+srv://vijayalakshmi:Tarun_3872@cluster0.udobscc.mongodb.net/')
        ...
        st.write(f"Successfully Uploaded to MogoDB !! ")
...
with col2:
    ...
    st.write ('''(Note:- Specific channel data is **Migrated** to the **:blue[MySQL]** database from the **:green[MongoDB]** database by **selection**.)''')
    ...
    for i in range(1, len(result['Channel_data']) - 1):
        ...
        engine = create_engine('mysql+mysqlconnector://root:Tarun_3872@localhost/youtube_db', echo=False)
...
st.write ('''(Note:- Based on the selected question from the dropdown,**analyzes the channel data** and generates a table of results.)''')
...
connect_for_question = pymysql.connect(host='localhost', user='root', password='Tarun_3872', db='youtube_db')
